...
import SeabugOnchain.Token (mkPolicy, mkTokenName)
...
import SeabugOnchain.Dao (daoValidator, mkValidator)
import SeabugOnchain.Lock (lockValidator, mkValidator)
import SeabugOnchain.Marketplace (mkValidator)
import SeabugOnchain.Types (LockAct, LockDatum, MarketplaceDatum, MintAct, NftCollection (..), NftId (..))
...
testLockScript =
  mkTestValidator
    ( $$(PlutusTx.compile [||SeabugOnchain.Lock.mkValidator||])
...
testMarketplaceScript =
  mkTestValidator
    $$(PlutusTx.compile [||SeabugOnchain.Marketplace.mkValidator||])
    $$(PlutusTx.compile [||toTestValidator||])

testDaoScript :: TestScript ( 'ForSpending BuiltinData BuiltinData)
testDaoScript =
  mkTestValidator
    ( $$(PlutusTx.compile [||SeabugOnchain.Dao.mkValidator||])
